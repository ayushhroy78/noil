...
export const OilConsumptionCalendar = ({ userId }: OilConsumptionCalendarProps) => {
  ...
  const motivationalMessages = [
    t("profile.dailyTip") + ": " + t("profile.healthTipText"),
  ];
  ...
  const handleAddEntry = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${newAmount}ml ${t("tracker.usageLoggedDesc")} - ${format(selectedDate, "MMM d, yyyy")}`
      });
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-950/50 dark:to-emerald-900/30 border-emerald-200/50">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-muted-foreground">{t("profile.daysTracked")}</span>
            </div>
            <p className="text-2xl font-bold text-emerald-700 dark:text-emerald-400">{stats.daysTracked}</p>
            <p className="text-xs text-muted-foreground">{t("profile.ofDays")} {stats.daysInMonth} {t("common.days")}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100/50 dark:from-blue-950/50 dark:to-blue-900/30 border-blue-200/50">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-muted-foreground">{t("profile.dailyAverage")}</span>
            </div>
            <p className="text-2xl font-bold text-blue-700 dark:text-blue-400">{stats.dailyAverage}{t("common.ml")}</p>
            <p className="text-xs text-muted-foreground">{t("profile.goal")}: {dailyGoal}{t("common.ml")}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100/50 dark:from-purple-950/50 dark:to-purple-900/30 border-purple-200/50">
          <CardContent className="pt-4 pb-3">
            ...
            <p className="text-2xl font-bold text-purple-700 dark:text-purple-400">{stats.totalThisMonth}{t("common.ml")}</p>
        ...
        <Card className="bg-gradient-to-br from-orange-50 to-orange-100/50 dark:from-orange-950/50 dark:to-orange-900/30 border-orange-200/50">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-muted-foreground">{t("profile.trackingStreak")}</span>
            </div>
            <p className="text-2xl font-bold text-orange-700 dark:text-orange-400">{stats.trackingStreak} {t("common.days")}</p>
            <p className="text-xs text-muted-foreground">{t("profile.keepItUp")}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent className="pt-4 pb-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">{t("profile.trackingProgress")}</span>
            <span className="text-xs text-muted-foreground">
              {stats.daysTracked}/{stats.daysInMonth} {t("common.days")} ({Math.round(trackingProgress)}%)
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3">
          ...
          <Card className="bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-950/30 dark:to-amber-950/30">
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium">{t("profile.bestDayTitle")}</span>
              </div>
              <p className="text-sm font-bold">{format(stats.bestDay.date, "MMM d")}</p>
              <p className="text-xs text-muted-foreground">{stats.bestDay.amount}{t("common.ml")} {t("profile.consumed")}</p>
            </CardContent>
          </Card>
        )}
        ...
        <Card className="bg-gradient-to-br from-pink-50 to-rose-50 dark:from-pink-950/30 dark:to-rose-950/30">
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-medium">{t("profile.dailyTip")}</span>
            </div>
        ...
        <CardContent className="pb-4">
          ...
          <div className="flex items-center justify-center gap-4 mt-4 text-xs">
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">{t("profile.optimal")}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded bg-amber-200" />
              <span className="text-muted-foreground">{t("profile.moderate")}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded bg-rose-200" />
              <span className="text-muted-foreground">{t("profile.high")}</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="pt-4 pb-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium">{t("tracker.dailyGoal")}</p>
              <p className="text-xs text-muted-foreground">{t("profile.setDailyGoal")}</p>
            </div>
      ...
      <Dialog open={entryDialogOpen} onOpenChange={setEntryDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div>
              <label className="text-sm font-medium mb-2 block">{t("profile.amountMl")}</label>
              <div className="flex gap-2">
                <Input
                  type="number"
                  value={newAmount}
                  onChange={(e) => setNewAmount(e.target.value)}
                  placeholder={t("profile.enterAmountMl")}
                />
              </div>
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">{t("profile.notesOptional")}</label>
              <Textarea
                value={newNotes}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddEntry} disabled={isSubmitting || !newAmount}>
              {isSubmitting ? t("common.saving") : t("profile.addEntry")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
