  ...
  return <div className="space-y-4">
      ...
      <Card className="bg-gradient-to-br from-primary/15 via-primary/10 to-primary/5 border-primary/20 overflow-hidden relative">
        ...
        <CardContent className="pt-6 relative">
          ...
          {nextTier && <div className="space-y-2">
      ...
      {earnedBreakdown.length > 0 && <Card>
          ...
          <CardContent className="space-y-3">
            {earnedBreakdown.sort((a, b) => b.total - a.total).map((breakdown, index) => <div key={breakdown.source} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      ...
                      <Progress value={breakdown.total / totalEarned * 100} className="w-20 h-1.5" />
                    </div>
                  </div>
                </div>)}
          </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            return <div key={tier.name} className={cn("flex items-center gap-3 p-2 rounded-lg transition-all", isCurrentTier && "bg-primary/10 ring-1 ring-primary/30", isCompleted && "opacity-60")}>
                  <Badge className={cn("w-16 justify-center", tier.bgColor, tier.color, isCompleted && "opacity-70")}>
                    {tier.name}
                  </Badge>
                  <div className="flex-1">
                    <Progress value={isCompleted ? 100 : isCurrentTier ? tierProgress : 0} className="h-2" />
                  </div>
